...
class emp_phone_duplicate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_url = urlbase.sit_emp() + "/emp/checkEmpName"
    ...
    def test_params_correct(self):
        ''' 正确的参数_不存在的邮箱'''
        payload = {"empId":self.s1,"empName":"ZHANGHAO3"}
    ...
    def test_param_now_correct(self):
        ''' 正确的参数_当前的邮箱'''
        payload = {"empId":self.s1,"empName":"ZHANGHAO2"}
        r2 = self.s.get(self.base_url, params=payload)
        self.result = r2.json()
        self.assertEqual(self.result['result'], True)
        self.assertEqual(self.result['resultObject'], None)
    ...
    def test_get_userinfo_wrong_empphone(self):
        ''' 错误的参数_存在的邮箱'''
        payload = {"empId":self.s1,"empName":"ZHANGHAO1"}
        r2 = self.s.get(self.base_url, params=payload)
        self.result = r2.json()
        self.assertEqual(self.result['result'], False)
        self.assertEqual(self.result['resultObject'], None)
